...
check_match <- function(df, rown, corpus, ...) {
  require(dplyr)
  require(textreuse)
  df <- as.tbl(df)
  id1 <- df[[rown, "a"]]
  ...
  doc2 <- corpus[[id2]]
  score <- df[[rown, "score"]]
  alignment <- align_local(doc2, doc1, ..., progress = FALSE)
  cat(id2, ":\n")
  cat(content(doc2))
  cat("\n\n")
  cat(id1, ":\n")
  ...
  cat("\n")
  cat("\nLongest matching subsequence, with edits shown:\n")
  print(alignment)
  invisible(alignment)
}
